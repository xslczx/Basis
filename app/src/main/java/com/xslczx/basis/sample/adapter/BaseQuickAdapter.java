...
public abstract class BaseQuickAdapter<T, K extends BaseViewHolder> extends BaseSelectionAdapter<K>
    ...
    public void removeAt(int position) {
        if (position < 0 || position >= getDataCount()) return;
        mData.remove(position);
        notifyItemRemoved(position);
        notifyItemRangeChanged(position, getDataCount());
    }
